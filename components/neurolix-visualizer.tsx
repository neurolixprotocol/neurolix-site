  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (rectB.bottom > -80 && rectB.top < window.innerHeight + 80) {
       ...
       // Spingiamo la blockchain ancora più in basso (70%) per allungare il tragitto visivo dell'animazione
       const chainY = isMob ? (h * 0.70) : (h * 0.72); const bs = isMob ? 15 : 20, gap = bs * 1.8;
      ...
      <section id="sceneB" className="relative" style={{ height: 'calc(var(--app-height, 100vh) * 3.6)' }}>
        <div className="sticky top-0 overflow-hidden flex flex-col" style={{ height: 'var(--app-height, 100vh)' }}>
          ...
          <div className="flex-1 relative w-full">
            ...
            <div id="hashline" className="absolute left-0 right-0 top-[86%] -translate-y-1/2 md:top-[89%] md:-translate-y-1/2 z-10 flex flex-col items-center gap-1.5 px-4 md:px-6 pointer-events-none opacity-0 transition-opacity duration-500">
