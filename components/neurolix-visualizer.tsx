  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (rectB.bottom > -80 && rectB.top < window.innerHeight + 80) {
        ...
        const cy = isMob ? Math.max(115, h * 0.26) : (h * 0.40);
        // Su mobile usiamo quasi tutta la larghezza dello schermo (meno 40px di margine totale)
        const encW = isMob ? (w - 40) : Math.min(w * 0.82, 560);
        // Altezza proporzionata a scatola/terminale reale anziché striscia verticale
        const encH = isMob ? 140 : Math.min(h * 0.42, 340);
        ...
        const chainY = isMob ? (h - 65) : (h * 0.86); const bs = isMob ? 15 : 20, gap = bs * 1.8;
      ...
      <section id="sceneB" className="relative h-[360vh]">
        <div className="sticky top-0 h-screen overflow-hidden flex flex-col">
          ...
          <div className="flex-1 relative w-full">
            ...
            <div id="hashline" className="absolute left-0 right-0 top-[60%] -translate-y-1/2 md:top-auto md:translate-y-0 md:bottom-[22vh] z-10 flex flex-col items-center gap-1.5 px-4 md:px-6 pointer-events-none opacity-0 transition-opacity duration-500">
